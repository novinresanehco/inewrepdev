...
function shopcats_output()
{
    ...
    $itpl  = new samaneh();
	if( !empty( $args['template'] ) && ctype_alnum( $args['template'] ) && file_exists( current_theme_dir. 'plugins\\shopcats\\' . $args['template'] . '.html' ) )
	{
		$itpl->load( current_theme_dir. 'plugins\\shopcats\\' . $args['template'] . '.html' );
		
	}	
	else if( file_exists( current_theme_dir . 'plugins\\shopcats_block-theme.html' ) )
	{
		$itpl->load( current_theme_dir. 'plugins\\shopcats_block-theme.html' );
	}
	else
	{
		$itpl->load( 'plugins/shopcats/block-theme.html' );
	}
        ...
        while ( $data       = $d->fetch( $categories ) )
        {
            ...
            $ddd['url']    = appendQs( $url, 'shopCategory', $data['categoryID'] );
            ...
            while ( $idata = $d->fetch( $subCategories ) )
            {
                $itpl->block( 'subCategories_' . $data['categoryID'], array(
                    ...
                    'suburl'          => appendQs( $url, 'shopCategory', $idata['categoryID'] )
                ) );
            }
        }
