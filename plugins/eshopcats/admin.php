...
if ( !defined( 'plugins_admin_area' ) OR !isset( $permissions['access_admin_area'] ) OR $permissions['access_admin_area'] != '1' )
    die( 'invalid access' );
if( !function_exists( 'get_eshopcats_templates' ) )
{
	function get_eshopcats_templates()
	{
		$themesGLob = glob( current_theme_dir . 'plugins\\eshopcats\\' . '*.htm*' );
		$themes     = array();
		foreach ( $themesGLob as $value )
		{
			$value    = str_replace( current_theme_dir. 'plugins\\eshopcats\\' , '', $value );
			$value 	  = explode( '\\', $value );
...
$information = array(
    'name'        => 'دسته بندی محوصلات فروشگاه فایل',
    ...
    'data'        => array( //لیست پارامترهای اختصاصی ماژول
        'title'      => array( 'name' => 'عنوان', 'value' => 'دسته بندی محصولات فروشگاه فایل' ),
		'template' => array( 'name' => 'قالب', 'value' => 'default', 'type' => 'select', 'options' => get_eshopcats_templates()  ),
...
if ( defined( 'methods' ) )
{
    ...
    {
        global $d;
        $d->Query( "UPDATE `plugins` SET `stat`='0' WHERE `name`='eshopcats' LIMIT 1" );
    ...
    {
        global $d;
        $d->Query( "UPDATE `plugins` SET `stat`='1',`sortable`=1 WHERE `name`='eshopcats' LIMIT 1" );
    ...
    {
        global $d;
        $q = $d->getrows( "SELECT `stat` FROM `plugins` WHERE `name`='eshopcats' LIMIT 1", true );
        if ( $q > 0 )
        ...
        else
        {
            ...
            $q                = $d->Query( "INSERT INTO `plugins` SET `name`='eshopcats',`title`='$information[name]',`stat`='0',`options`='$options'" );
    ...
    function uninstallop()
    {
        ...
        {
            $q = $d->Query( "DELETE FROM `plugins` WHERE `name`='eshopcats' LIMIT 1" );
