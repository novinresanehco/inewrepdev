...
function eshoplistproducts_output()
{
    ...
    $itpl  = new samaneh();
	if( !empty( $args['template'] ) && ctype_alnum( $args['template'] ) && file_exists( current_theme_dir. 'plugins\\eshoplistproducts\\' . $args['template'] . '.html' ) )
	{
		$itpl->load( current_theme_dir. 'plugins\\eshoplistproducts\\' . $args['template'] . '.html' );
		
	}	
	else if( file_exists( current_theme_dir . 'plugins\\eshoplistproducts_block-theme.html' ) )
	{
		$itpl->load( current_theme_dir. 'plugins\\eshoplistproducts_block-theme.html' );
	}
	else
	{
		$itpl->load( 'plugins/eshoplistproducts/block-theme.html' );
	}
        if ( isset( $_GET['shopCategory'] ) && is_numeric( $_GET['shopCategory'] ) )
        {
            $shopCategory = ( int ) $_GET['shopCategory'];
            $query        = "SELECT * FROM `eproducts` WHERE `delete`=0 AND `category`='$shopCategory'";
            $queryCount   = "SELECT COUNT(*) as total FROM `eproducts` WHERE `delete`=0 AND `category`='$shopCategory'";
        }
        else
        {
        ...
        if ( isset( $_GET ) && is_array( $_GET ) )
        {
            foreach ( $_GET as $key => $value )
            {
                if ( !empty( $value ) && substr( $key, 0, 7 ) == 'search_' )
                {
                    ...
                    {
                        $query .= " AND `eproductID` IN ( SELECT `eproductID` FROM `eproduct_fields` WHERE `fieldID`='$key'  AND `value` LIKE '%$value%' ) ";
                        $queryCount .= " AND `eproductID` IN ( SELECT `eproductID` FROM `eproduct_fields` WHERE `fieldID`='$key'  AND `value` LIKE '%$value%' ) ";
                    }
                }
            }
        }
        ...
        while ( $data           = $d->fetch( $products ) )
        {
            ...
            {
                $itpl->block( 'productImages_' . $data['eproductID'], array(
            ...
            {
                $Fdata = $d->Query( "SELECT `value` FROM `product_fields` WHERE `eproductID`='$data[eproductID]' AND `fieldID`='$field[fieldID]' LIMIT 1" );
                $Fdata = $d->fetch( $Fdata );
                $value = $Fdata['value'];
                if ( !empty( $value ) )
                {
                    $itpl->block( 'productFields_' . $data['eproductID'], array(
