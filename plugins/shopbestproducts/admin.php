...
if ( !defined( 'plugins_admin_area' ) OR !isset( $permissions['access_admin_area'] ) OR $permissions['access_admin_area'] != '1' )
    die( 'invalid access' );
if( !function_exists( 'get_shopbestproducts_templates' ) )
{
	function get_shopbestproducts_templates()
	{
		$themesGLob = glob( current_theme_dir . 'plugins\\shopbestproducts\\' . '*.htm*' );
		$themes     = array();
		foreach ( $themesGLob as $value )
		{
			$value    = str_replace( current_theme_dir. 'plugins\\shopbestproducts\\' , '', $value );
			$value 	  = explode( '\\', $value );
...
$information = array(
    ...
    'data'        => array( //لیست پارامترهای اختصاصی ماژول
        'title'      => array( 'name' => 'عنوان', 'value' => 'بهترین محصولات فروشگاه' ),
		'template' => array( 'name' => 'قالب', 'value' => 'default', 'type' => 'select', 'options' => get_shopbestproducts_templates()  ),
...
if ( defined( 'methods' ) )
{
    ...
    {
        global $d;
        $d->Query( "UPDATE `plugins` SET `stat`='0' WHERE `name`='shopbestproducts' LIMIT 1" );
    ...
    {
        global $d;
        $d->Query( "UPDATE `plugins` SET `stat`='1',`sortable`=1 WHERE `name`='shopbestproducts' LIMIT 1" );
    ...
    {
        global $d;
        $q = $d->getrows( "SELECT `stat` FROM `plugins` WHERE `name`='shopbestproducts' LIMIT 1", true );
        if ( $q > 0 )
        ...
        else
        {
            ...
            $q                = $d->Query( "INSERT INTO `plugins` SET `name`='shopbestproducts',`title`='$information[name]',`stat`='0',`options`='$options'" );
    ...
    function uninstallop()
    {
        ...
        {
            $q = $d->Query( "DELETE FROM `plugins` WHERE `name`='shopbestproducts' LIMIT 1" );
