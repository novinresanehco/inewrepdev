...
function recentpostbycatParser( $tpl )
{
    ...
    if ( preg_match_all( '#\[recentPostByCat\_([0-9]+)(\_([a-z0-9\-]+))?\_([0-9]+)\]#Ui', $tpl, $matches ) )
    {
        for ( $i = 0, $c = count( $matches[0] ); $i < $c; $i++ )
        {
            $category           = $matches[1][$i];
    ...
    if ( preg_match_all( '#\[recentPostByCat\_([0-9]+)(\_([a-z0-9\-]+))?\_([0-9]+)\_([0-9]+)\]#Ui', $tpl, $matches ) )
    {
        for ( $i = 0, $c = count( $matches[0] ); $i < $c; $i++ )
        {
            $category           = $matches[1][$i];
...
function recentpostbycat_output()
{
	...
	*/
    ...
    $offset = ( isset( $args['offset'] ) && is_numeric( $args['offset'] ) && $args['offset'] > 0 ) ? $args['offset'] : 0;
    $itpl  = new samaneh();
    ...
    $iq         = $d->Query( "SELECT * FROM `data` WHERE 
        (`cat_id`='$catId'
            ...
            AND `date` <= '$ctimestamp' AND (`expire`='0' OR `expire`='' OR `expire`>$ctimestamp) AND (`show`!='4' || `show`='2') ORDER BY `id` DESC LIMIT $offset,$count" );
    $first      = true;
    $ivx = 0;
    ...
    while ( $idata      = $d->fetch( $iq ) )
    {
        ...
        {
            $itpl->assign( array(
                'secondTitle' => $idata['title'],
                'secondUrl'   => $url,
                'secondText'  => $idata['text'],
                'secondDate'  => $date,
                'secondCat'   => $cat,
                'secondImage' => $idata['timage'],
            ) );
            //continue;
        }
        if ( $ivx == 2 )
        {
            $itpl->assign( array(
                'thirdTitle' => $idata['title'],
                'thirdUrl'   => $url,
                'thirdText'  => $idata['text'],
                'thridDate'  => $date,
                'thirdCat'   => $cat,
                'thirdImage' => $idata['timage'],
            ) );
            //continue;
        }
