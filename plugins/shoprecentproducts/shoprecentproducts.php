...
function shoprecentproducts_output()
{
    ...
    $itpl  = new samaneh();
	if( !empty( $args['template'] ) && ctype_alnum( $args['template'] ) && file_exists( current_theme_dir. 'plugins\\shoprecentproducts\\' . $args['template'] . '.html' ) )
	{
		$itpl->load( current_theme_dir. 'plugins\\shoprecentproducts\\' . $args['template'] . '.html' );
		
	}	
	else if( file_exists( current_theme_dir . 'plugins\\shoprecentproducts_block-theme.html' ) )
	{
		$itpl->load( current_theme_dir. 'plugins\\shoprecentproducts_block-theme.html' );
	}
	else
	{
		$itpl->load( 'plugins/shoprecentproducts/block-theme.html' );
	}
