...
$acr = $d->Query("SELECT tvote,nov FROM `$tbl` WHERE id='$id'");
$acr = $d->fetch($acr);
$rate = number_format(($acr['tvote']/$acr['nov']),2,".","");
$ec = $rate;
$rate= round($rate);
for($i=0;$i<$rate;$i++){
echo '<img src="'.$config['site'].'theme/core/'.$config['theme'].'/img/rate_on.gif" border="0">';
}
$rate= 5-$rate;
if($rate !== 0){
for($i=0;$i<$rate;$i++){
echo '<img src="'.$config['site'].'theme/core/'.$config['theme'].'/img/rate_off.gif" border="0">';
}
}
echo " ".$ec;